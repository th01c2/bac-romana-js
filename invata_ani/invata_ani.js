const rawLiteraryData = [
    ...
    { opera: "Riga Crypto și Lapona Enigel", author: "Ion Barbu", category: "baladă cultă", period: "modernism", year: 1924, currents: ["modernism"] },
...
function generateAllGameQuestions() {
    ...
    const operasWithYears = rawLiteraryData.filter(item => item.year);
...
function handleSubmitAnswer() {
    ...
    if (normalizedUserAnswer === normalizedCorrectAnswer) {
        ...
        feedbackMessage.textContent = "Mai încearcă.";
        feedbackMessage.className = 'feedback-message incorrect';
        answerInput.focus();
    }
    updateScoreDisplay();
...
function handleGiveUp() {
    ...
    feedbackMessage.textContent = `Răspunsul corect: ${correctAnswer}`;
    feedbackMessage.className = 'feedback-message incorrect';
    incorrectScore++;
    setAnswerControlsState(true);
    timerId = setTimeout(nextQuestion, 2000);
    updateScoreDisplay();
}
...
function endGame() {
    ...
    finalScoreMessage.innerHTML = `Scor: ${correctScore} corecte, ${incorrectScore} greșite.`;
}
