const rawLiteraryData = [
    ...
    { opera: "Riga Crypto și Lapona Enigel", author: "Ion Barbu", category: "baladă cultă", period: "modernism ermetic", year: 1924, currents: ["modernism", "baladă cultă"] },
    ...
    { opera: "Testament", author: "Tudor Arghezi", category: "poem liric", period: "modernism (estetica urâtului)", year: 1927, currents: ["modernism", "estetica uratului"] },
...
function generateAllGameQuestions() {
    ...
    rawLiteraryData.forEach(item => {
        generatedQuestions.push({
            ...
            acceptedAnswers: [item.period, ...(item.currents || [])].filter(Boolean).map(s => s.trim())
        });
        if (item.currents && item.currents.length > 0) {
            item.currents.forEach(currentKeyword => {
                ...
                } else if (normKeyword !== "balzacian" || !normalizeString(item.category).includes("roman")) {
                    generatedQuestions.push({
                        type: 'current-to-opera',
        ...
        for (const acceptedAns of currentQuestion.acceptedAnswers) {
            ...
            if (!isCorrect && normalizedAcceptedAns.includes(normalizedUserAnswer)) {
                if ((normalizedAcceptedAns.startsWith("neo") || normalizedAcceptedAns.startsWith("post")) &&
                    ...
                    !normalizedUserAnswer.startsWith("neo") && !normalizedUserAnswer.startsWith("post")) {
...
function handleGiveUp() {
    ...
    feedbackMessage.textContent = `Răspunsul corect: "${correctAnswerTextToShow}"`;
    feedbackMessage.className = 'feedback-message incorrect';
    incorrectScore++;
    setAnswerControlsState(true);
...
function endGame() {
    ...
    finalScoreMessage.innerHTML = `Scor final: ${correctScore} corecte, ${incorrectScore} greșite.`;
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (answerInput) {
        answerInput.addEventListener('keypress', (event) => {
            if (event.key === 'Enter' && !submitAnswerButton.disabled) handleSubmitAnswer();
        });
    }
